...
8. **Current-vs-target heatmap.** Add a vendor-neutral *Have / Partial / Gap* overlay per box so the model can drive a roadmap rather than read as all-target-state. (Review §4.1.3 / §3.1.2.)
...
10. **Add a CoSAI / Google-SAIF crosswalk** alongside the existing OWASP/NIST/ATLAS mapping — cheap credibility win for assurance reviewers. (Review §3.3.)
